...
def test_exact_duplicates_collapse_in_queue():

    """Exact duplicates (same normalized URL) must collapse into a single queue item."""

    profile = _profile()

    v1 = _vac(sid="hh_101", title="Backend Engineer", desc="python", company="Acme Corp", source="hh", job_url="https://hh.ru/vacancy/101?utm_source=search")

    v2 = _vac(sid="hh_102", title="Backend Engineer", desc="python", company="Acme Corp", source="hh", job_url="https://hh.ru/vacancy/101?ref=telegram")
    ...
    items = build_queue_items(
        ...
        {v1.stable_id(): m, v2.stable_id(): m},

        {v1.stable_id(): d, v2.stable_id(): d},

    )

    assert len(items) == 1

    assert items[0].canonical_id is not None

    assert len(items[0].canonical_id) > 0

    assert items[0].representative_vacancy_stable_id in (v1.stable_id(), v2.stable_id())





def test_probable_duplicates_stay_separate_in_queue():

    """Probable duplicates (same title/company, different URLs) must remain separate items."""

    profile = _profile()

    v1 = _vac(sid="rem_201", title="Senior Python Developer", desc="python", company="Globex", source="remoteok", job_url="https://remoteok.com/jobs/201")

    v2 = _vac(sid="hima_202", title="Senior Python Developer", desc="python", company="Globex", source="himalayas", job_url="https://himalayas.app/jobs/202")
    ...
    item_sids = {it.vacancy_stable_id for it in items}

    assert item_sids == {v1.stable_id(), v2.stable_id()}





def test_unmapped_canonical_passes_through_in_queue():

    """An unmapped vacancy without pre-existing canonical passes through and receives a canonical_id."""

    profile = _profile()

    v = _vac(sid="unmapped_301", title="Unique Specialist", desc="python", company="SoloCorp", source="weworkremotely", job_url="https://weworkremotely.com/jobs/301")
    ...
    assert items[0].vacancy_stable_id == v.stable_id()

    assert items[0].canonical_id is not None

    assert items[0].canonical_id.startswith("canonical_")
