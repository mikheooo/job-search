...
def test_submitted_requires_fingerprint_evidence(clean_db):
    """Transition READY_TO_SUBMIT -> SUBMITTED with confirm_submit=True but without fingerprint is rejected."""
...
def test_submitted_with_fingerprint_succeeds(clean_db):
    """Transition READY_TO_SUBMIT -> SUBMITTED with confirm_submit=True and valid fingerprint succeeds."""
    app_id = "app_test_with_fp"
    db.save_hh_application({
        "application_id": app_id,
        "state": HHApplicationState.READY_TO_SUBMIT.value,
        "vacancy_stable_id": "hh:12345678",
    ...
    res = transition_application(
        application_id=app_id,
        to_state=HHApplicationState.SUBMITTED,
        reason="test_with_fingerprint",
        confirm_submit=True,
        evidence={"fingerprint": "valid_sha256_package_fingerprint_12345"},
    )
