...
Осталось узкое место, о котором надо знать: если вакансии нет в таблице
`vacancies`, заголовок неизвестен, проверка пропускается и в лог падает
WARNING. Сделать это жёстким отказом (не знаем, куда откликаемся — не
откликаемся) можно, но это меняет поведение пути отправки, поэтому я так не
сделал без решения Миши.

Тестов в файле теперь 51.
