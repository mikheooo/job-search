...
>
> - **2026-09-10 — 2026-09-16: находки №16–№34.** Пять критических на путях
...
>   выключало сверку вакансии. Тестов в файле теперь 137, полный сьют
>   **1675 passed** (`errors=0 failures=0 skipped=0`, junitxml).
>   Два пункта ждут решения Миши, и оба — про анкету:
>   **№27** (инвариант «форма изменилась → стоп» в проде недостижим: замер
>   показал, что он недостижим дважды — отпечаток не передаёт ни один боевой
>   вызов, и самих анкет в проде никто не создаёт; а если включить наивно, семь
>   правок из восьми на стороне работодателя дадут ложный стоп) и
>   **№34** (на вакансии со скрининговыми вопросами живая проверка говорит
>   READY, а все 11 гейтов — «All 11 gates passed successfully», потому что
>   гейт 8 читает вопросы из пакета, а не со страницы; вопросов при этом никто не
>   читал). Подробности — раздел «Находки №8–№29» ниже, находки №30–№33 сразу за
>   ним и замеры внутри №27 и №34.
...
### 34. На вакансии со скрининговыми вопросами все 11 гейтов говорят «готово», а вопросов никто не читал (средне-высокая) — **НАЙДЕНО, НЕ ИСПРАВЛЕНО**
...
**Почему не чиню.** Правка — научить живой осмотр отдавать признак «на странице
есть контролы `task_*`, которые мы не заполняем», и валить гейт 8 по нему. Это
меняет поведение на самом опасном пути, а калибровать нечем: живой страницы нет.
Тот же довод, что в №27: непроверяемая правка на пути клика хуже записанной
находки. **Нужно решение Миши** (и, если делать, — разрешение на одну пробу на
настоящей вакансии).

Практический смысл для Миши: сегодня бот не умеет откликаться на вакансии со
скрининговыми вопросами. По заметке в разделе E2E ниже, живая вакансия даёт
`application_type=screening_questions` и 6 вопросов — то есть это не редкий
случай, а обычный. Анкеты при этом заполнялись руками: две строки в `state.db` от
2026-08-30, обе `SUBMITTED`, id семантические — боевой разбор таких id не выдаёт.
