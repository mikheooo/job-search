...
### 8. Kill-switch `SUBMIT_ALLOWED` не выключается переменной окружения (высокий)
...
**Не исправлено сознательно.** Правка меняет поведение продакшн-гейта, а `.env`
с `true` похож на осознанную подготовку к реальной отправке. Трогать гейт,
пока машина наведена на живых работодателей, не стал. Вариант фикса, когда
будет понятно, что отправка опять выключена:

```python
# читать env в момент проверки, а не замороженный config
submit_allowed = os.getenv("SUBMIT_ALLOWED", "").strip().lower() in ("1", "true", "yes")
if os.getenv("SUBMIT_ALLOWED") is None:          # явно не задано - тогда из .env
    submit_allowed = bool(getattr(config, "SUBMIT_ALLOWED", False))
```

Пока переменная задана явно — она побеждает. Kill-switch начинает работать.
