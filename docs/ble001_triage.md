...
>
> - **2026-09-10 — 2026-09-14: находки №16–№29.** Пять критических на путях
>   клика и записи `SUBMITTED` (№19, №20, №21, №22, №25) и высокая №29 —
>   раннер докладывал «REAL HH SUBMIT: 1», писал `SUBMITTED` и слал
>   уведомление, когда исполнителя не было вообще (Chrome не поднят). Тестов
>   в файле теперь 118, полный сьют **1656 passed**.
...
>   Подробности — раздел «Находки №8–№29» ниже.
...
5. ruff: `submission_recovery.py` 15 → **10** (минус пять `S110`, BLE001 остался 9 —
правило срабатывает на сам `except`, менялось тело); `cli.py` 20 → 20, паритет.
