...
>   выключало сверку вакансии. Тестов в файле теперь 148, полный сьют
>   **1686 passed, 0 failed** (`errors=0 failures=0 skipped=0`, junitxml).
>
...
>   Дубли переведены на маркер скрипта, проверено встречно.
>
...
- `tests/test_ble001_fail_closed.py`: **137 → 148** тестов (№34 дала 10, №36 — один).
- Полный сьют: **1686 passed, 0 failed** (`--junitxml`; 1685 после №34 и 1686 после №36).
...
Остальные дубли с таким же рыхлым разбором (`test_stage34_*`, `test_stage36_*`,
`test_phase2_1_*`, `test_stage51_*`, `test_step25_*`) не тронуты: этот случай их
не задел, но грабли те же. Вывод на будущее: **комментарий внутри JS — часть
контракта**, пока дубль смотрит на подстроки.
