...
>
> - **2026-09-10 — 2026-09-15: находки №16–№30.** Пять критических на путях
>   клика и записи `SUBMITTED` (№19, №20, №21, №22, №25), высокая №29 —
>   раннер докладывал «REAL HH SUBMIT: 1», писал `SUBMITTED` и слал
>   уведомление, когда исполнителя не было вообще (Chrome не поднят), — и
>   средняя №30: след аудита отклика молча терял целые категории, а при
>   полном отказе печатал «No audit events found.» с кодом 0. Тестов в файле
>   теперь 124, полный сьют **1662 passed** (`errors=0 failures=0 skipped=0`,
>   junitxml).
...
>   Подробности — раздел «Находки №8–№29» ниже и находка №30 сразу за ним.
